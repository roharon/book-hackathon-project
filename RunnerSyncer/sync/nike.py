...
class TRKPoint:
   ...
   """.format(
            self.latitude, self.longitude, self.element, to_datetime(self.time)
        )
...
def to_gpx(data):
    ...
    return GPX_TEMPLATE.format(
        start_time, name, "".join([str(point) for point in points])
    )


def fetch_activity_to_gpx(activity_id: str, access_token: str) -> str:
    nike_activity_url = (
        "https://api.nike.com/sport/v3/me/activity/{}?metrics=ALL".format(activity_id)
    )
...
def fetch_activity_ids(access_token: str) -> str:
    nike_activity_list_url = (
        "https://api.nike.com/sport/v3/me/activities/before_time/{}".format(
